...
public class Movement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (isGrounded) // Ground move
        {
            ...
            {
                // Jump away
                _velocity += Vector3.up * _jumpStrength;
            }
        }
        else // Air move
        {
            ...
            _velocity += Vector3.down * (_gravityAmount * dt);
        }
    ...
    private bool IsGrounded(out Vector3 groundNormal)
    {
        ...
        foreach (Transform t in _groundedRayPositions)
        {
            ...
            if (Physics.Raycast(t.position, Vector3.down, out RaycastHit hit, 0.51f, ~_excludedLayers))
            {
